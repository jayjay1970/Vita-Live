...
function gmpBindTinyMceUpdate() {
	if(!g_gmpTinyMceEditorUpdateBinded && typeof(tinyMCE) !== 'undefined' && tinyMCE.editors) {
		...
		} else if(tinyMCE.editors.shapeDescription) {
			tinyMCE.editors.shapeDescription.onKeyUp.add(function(){
				var shape = gmpGetCurrentMarker();
				if(!shape) {
					_gmpCreateNewMapMarker();
					shape = gmpGetCurrentMarker();
				}
				if(shape) {
